    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            ...
            if ini_pose is None:
                ...
                _, rvec0, T0 = cv.solvePnP(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP)
...
def batch_transform_3d(P, pts3d, angle_axis=True):
    # pts3d.size =  (N,3)
    # P.size = (B,6)
    n = pts3d.size(0)
    bs = P.size(0)
